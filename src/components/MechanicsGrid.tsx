...
export default function MechanicsGrid() {
  ...
  const s = m.gamificacaoPage.mechanics;
      ...
      <div className="container mx-auto max-w-7xl px-4">
        <div className="mx-auto mb-20 max-w-3xl text-center">
          ...
          <h2 className="mb-6 font-heading text-4xl font-black text-white md:text-5xl">
            {s.titleBefore}{" "}
            <span className="bg-gradient-to-r from-yoobe-purple to-fuchsia-600 bg-clip-text text-transparent">{s.titleGradient}</span>{" "}
            {s.titleAfter}
          </h2>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {s.items.map((item, i) => (
              <div
                key={item.id}
                className={`group relative overflow-hidden rounded-[2rem] border border-white/5 bg-[#141b2d] p-8 transition-all duration-300 ${BORDERS[i]} hover:-translate-y-2 hover:shadow-2xl`}
              >
                <div
                  className={`absolute right-0 top-0 h-48 w-48 rounded-full bg-gradient-to-bl ${GRADIENTS[i]} opacity-0 blur-3xl transition-opacity duration-500 group-hover:opacity-100`}
                ></div>
