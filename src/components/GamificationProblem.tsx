      ...
      <div className="container relative z-10 mx-auto px-4">
        <div className="mx-auto mb-16 max-w-3xl text-center">
          ...
          <h2 className="mb-6 font-heading text-3xl font-black text-white md:text-5xl">
            {p.title} <span className="bg-linear-to-r from-yoobe-purple to-fuchsia-600 bg-clip-text text-transparent">{p.titleGradient}</span> {p.titleAfter}
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          {p.cards.map((card, i) => (
            <div
              key={card.title}
              className={`rounded-2xl border border-white/10 bg-surface-panel p-8 transition-all hover:-translate-y-1 ${CARD_BORDER[i]}`}
            >
