...
export default function GamificationTrends() {
  ...
  const t = m.gamificacaoPage.trends;
      ...
      <div className="container relative z-10 mx-auto max-w-6xl px-4">
        <div className="mx-auto mb-16 max-w-3xl text-center">
          ...
          <h2 className="mb-6 font-heading text-3xl font-black text-white md:text-5xl">
            {t.titleBefore}{" "}
            <span className="bg-gradient-to-r from-yoobe-purple to-fuchsia-600 bg-clip-text text-transparent">{t.titleGradient}</span>
            {t.titleAfter ? ` ${t.titleAfter}` : ""}
          </h2>
          <p className="text-lg text-white/60">{t.sub}</p>
        </div>

        <div className="group relative mb-16 flex flex-col items-center justify-around gap-8 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-r from-[#121824] to-[#1a1025] p-8 text-center shadow-2xl md:flex-row">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {t.items.map((trend, idx) => (
            <div
              key={idx}
              className="group rounded-2xl border border-white/10 bg-[#121824] p-8 transition-all hover:-translate-y-2 hover:border-yoobe-purple/50"
            >
