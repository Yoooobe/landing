...
function homeBlocks(
  ...
  return [
    ...
    {
      _key: "stats-highlights",
      _type: "statsBlock",
      title: stats.statsBar.title,
      items: stats.statsBar.items
        .filter((item) => item.group === "operational" && item.highlight)
        .map((item) => ({
    ...
    },
    {
      ...
      items: rest.testimonials.items.slice(0, 2).map((item) => ({
    ...
    },
    {
      ...
      bullets: [...rest.apiSection.bullets.slice(0, 4)],
...
function icpVerticalBlocks(page: IcpVerticalPage): MarketingPageDoc["content"] {
  return [
    ...
    },
    {
      ...
      primaryHref: withBasePathHref(page.cta.primaryHref),
      secondaryLabel: page.cta.secondaryLabel,
      secondaryHref: page.cta.secondaryHref ? withBasePathHref(page.cta.secondaryHref) : undefined,
      showLeadForm: true,
      leadFormVariant: "marketing",
    },
  ];
  ...
  switch (slug) {
    ...
    default: {
      ...
      if (vertical) {
        ...
        return {
          _id: `fallback-marketing-page.${locale}.${slug}`,
          title: locale === "en" ? vertical.titleEn : vertical.titlePt,
          slug,
          locale,
          summary:
            locale === "en"
              ? "Fallback marketing page for an ICP vertical."
              : "Pagina de fallback para uma vertical de ICP.",
          seo: {
