...
2. **Skill-Factory** — quando acionado para isso, o orquestrador **le cada produto e o
   transforma em uma skill autocontida do Claude Code, instalada em `~/.claude/skills/`**,
   para que a pessoa use a skill em qualquer projeto. Esse e o papel principal do
   orquestrador hoje. Procedimento completo: [.claude/skill-factory.md](.claude/skill-factory.md).
...
Quando o usuario acionar um comando, carregue o arquivo `.claude/commands/{comando}.md` correspondente e siga as instrucoes dele. Se o usuario pedir algo em linguagem natural sem comando explicito, escolha o comando mais provavel pela intencao:
...
[.claude/skill-factory.md](.claude/skill-factory.md). Resumo do contrato:

- As 7 pastas de produto continuam independentes e com suas funcoes (os slash commands
  locais seguem valendo). O orquestrador **nao altera os produtos**.
- Cada produto vira uma **skill autocontida** (toda a inteligencia da pasta copiada para
  dentro da skill) instalada em `~/.claude/skills/{nome}/` — funciona em qualquer projeto
  sem depender deste repo nem de `HUMAN_AGENT_LAB_HOME`.
- A criacao e instalacao sao feitas **pelo proprio Claude** (padrao da skill
  `anthropic-skills:skill-creator`), nunca pelos instaladores antigos em `install/`.
- Fluxo obrigatorio: ler -> apresentar a fila -> **esperar OK explicito** -> criar e
  instalar uma skill por vez, em fila -> relatorio final.

## Regras gerais

- Fale com o usuario em portugues.
- O ambiente oficial de treinamento e execucao e o Claude Code, com compatibilidade obrigatoria para Mac e Windows.
...
- Quando o usuario pedir para usar as skills em qualquer pasta, use o modo **Skill-Factory** (`/instalar` / `.claude/skill-factory.md`): crie e instale cada produto como skill autocontida em `~/.claude/skills/`. **Nao** use `install/install.sh`, `install/install.ps1` nem `install/install-global-commands.py` — estao aposentados.
...
- Ao finalizar qualquer execucao que gere arquivos, informe a **pasta final** em link clicavel e liste os arquivos gerados (nao-`.md`) em links clicaveis. Nao liste `.md` individualmente.
...
- Todo fluxo com Routines deve ter setup claro a partir do Claude Code, com instrucoes seguras para Mac e Windows quando houver parte local.
...
- Excecao compartilhada: se qualquer skill precisar desdobrar uma pasta com texto + imagens em IG Feed, IG Stories e LinkedIn Feed, acione o Human Social lendo `.claude/commands/social.md` ou `.claude/commands/desdobrar.md`.
- Antes de executar comandos pagos, externos ou que dependem de credenciais, confirme se a configuracao necessaria existe.
- Se uma pasta tiver `CLAUDE.md`, leia esse arquivo primeiro. Se tiver `COMECE-AQUI.md`, use como guia humano/operacional. Se tiver `SKILL.md` ou comando interno, preserve o fluxo existente.
