...
- **Backend incluído no repo:** existe um serviço mínimo em [`nano-banana-api/`](../nano-banana-api/) (Gemini API / `gemini-2.5-flash-image` via [`@google/genai`](https://github.com/googleapis/js-genai), deploy na Vercel). Segue o [README do pacote](../nano-banana-api/README.md); o valor do secret deve ser a URL completa do endpoint, por exemplo `https://<projeto>.vercel.app/api/generate`. O backend faz *rewrite* de `/` e `/api` para `/api/generate`, para não dar 404 na Vercel se alguém configurar só o domínio.
- **Contrato sugerido:** `POST` JSON `{ "prompt": string }`; opcionalmente `aspectRatio` (ex.: `16:9`, `1:1`) e `style` (pistas visuais, anexadas ao prompt no servidor). Resposta `image/png` / `image/jpeg` **ou** JSON com `imageBase64` / `base64` / `image` (base64 ou data URL). Detalhes em [`nano-banana-api/api/generate.ts`](../nano-banana-api/api/generate.ts) e no comentário de [`src/sanity/assetSources/nanoBananaImageSource.tsx`](../src/sanity/assetSources/nanoBananaImageSource.tsx).
- **Fluxo no Studio:** geração em duas fases — **pré-visualização** (blob local) e **Aplicar ao campo** (upload para o Sanity). Há **presets** de prompt, **proporção** (`aspectRatio`), **pistas de estilo** (`style`), **Regenerar**, **timeout** (~2 min) e **AbortController** no `fetch` (aviso se cancelar ou expirar).
- **Segurança:** o endpoint configurado na variável é chamado a partir do browser dos editores; trata-o como URL pública. A API real deve validar pedidos, aplicar **rate limit** (o backend em [`nano-banana-api`](../nano-banana-api/) limita pedidos por IP por minuto, por instância), manter **chaves de fornecedores de IA só no servidor** e configurar **CORS** apenas para as origens do Studio (localhost e o domínio onde o site é servido).
- **Checklist pós-geração (editorial):** ao **Aplicar ao campo**, imagens **> ~80 KB** são convertidas para **JPEG** em várias passagens (lado máximo até ~1920 px → reduções até ~512 px se ainda excederem ~500 KB) antes do `assets.upload`, para evitar **502** no gateway Sanity. Depois define **texto alternativo** (`alt`) onde o schema o permitir; verifica **proporção** e revisão humana para marca e precisão.
- **Quotas / custos:** faturação Gemini/Google conforme a conta; sem créditos a API devolve erro — ver [preços](https://ai.google.dev/pricing) e quotas do projeto.
...
- **Ícone (Lucide):** lista curada em [`src/config/marketing-icon-manifest.ts`](../src/config/marketing-icon-manifest.ts) (constante `MARKETING_ICON_CATALOG_LICENSE`), escolha visual no Studio via [`IconPickerInput`](../src/sanity/components/IconPickerInput.tsx). O site resolve o ícone em [`marketing-icon-registry.tsx`](../src/lib/marketing-icon-registry.tsx) (imports explícitos para *tree-shaking*).
- **SVG personalizado (opcional):** ficheiro `image/svg+xml` que **substitui** o Lucide quando preenchido. O SVG é obtido pela URL do asset no build (`customSvgUrl` na query GROQ) e **sanitizado** com DOMPurify (`isomorphic-dompurify`) antes de renderizar — ver [`sanitize-svg.ts`](../src/lib/sanitize-svg.ts). Testes mínimos: `npm run test:svg`.

Para acrescentar novos ícones Lucide ao catálogo: edita o manifest, regista o mesmo export em `marketing-icon-registry.tsx` e segue a governança em [`AGENTS.md`](../AGENTS.md) (secção de curadoria). Famílias e licenças: [`docs/icon-libraries.md`](icon-libraries.md).
...
**Dieta de copy (2026-06):** a home foi enxugada de 26 para 23 secções — `PricingSection` saiu (a página `/pricing` continua linkada no nav/footer) e os `SectionLeadCta` foram reduzidos de 3 para 1 (fica só `afterPlatform`). O `homeBlocks()` em `src/sanity/lib/marketingPages.ts` reflete a mesma composição; ao editar `marketingPage.home` no Studio, não reintroduzir blocos de pricing nem CTAs intermediários extra. As regras de copy (frases canónicas, estatística única por site, rótulo "exemplo de mercado") estão em `docs/knowledge-base/notebooklm/icp-messaging-guide.md` → secção "Frases canónicas".
...
**Fallbacks:** quando não há URL válida de asset no Sanity, vários blocos usam screenshots estáticos em `/public/screens/` (WebP) ou o `FeatureScreensCarousel` com esses assets. O hero da página Plataforma e secções de gamificação/inteligência também usam essas capturas — não há mais o módulo `PlatformMockupScreens` (mockups JSX removidos). Os mockups JSX vazios também foram eliminados de `PlatformTabs` (fallbacks `admin-dashboard-v3.webp`, `member-store-home.webp`, `admin-campaign-config-desktop.webp`), do case Hapvida em `EnterpriseCases` (`screens/hapvida/hapvida-vendas-on-login.webp`) e do card Workvivo em `DedicatedIntegrations` (`workvivo/workvivo-feed-shoutout.webp`). Em `/plataforma`, `SecurityEnterprise` (`screens/security-panel.webp`, gerada por IA) e `LogisticsFulfillment` (`screens/member-orders.webp`) renderizam sempre o painel com fallback local em vez de o omitir. Os campos Sanity continuam com prioridade quando preenchidos no Studio.
