...
A composição nativa em [`HomePage.tsx`](../src/components/HomePage.tsx) tem **8 secções** (2026-06): hero → trust strip → platform tabs → why (stats + diferenciais) → how it works (incl. duality motor/API) → enterprise cases + depoimentos → integrações/API → CTA final (formulário único). Secções removidas da home pública: bento, store, management, AI roadmap, gamification summary, clients grid completo e stats internacionais extensos — o fallback `homeBlocks()` em [`marketingPages.ts`](../src/sanity/lib/marketingPages.ts) espelha esta ordem reduzida.
