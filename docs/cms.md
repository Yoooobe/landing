...
- **Checklist pós-geração (editorial):** ao **Aplicar ao campo**, imagens **> ~80 KB** são convertidas para **JPEG** com lado máximo ~1920 px (segunda passagem mais agressiva se ainda forem grandes) antes do `assets.upload`, para evitar **502** no gateway Sanity. Depois define **texto alternativo** (`alt`) onde o schema o permitir; verifica **proporção** e revisão humana para marca e precisão.
