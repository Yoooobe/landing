...
- **Checklist pós-geração (editorial):** ao **Aplicar ao campo**, imagens grandes são **comprimidas no browser** antes do `assets.upload` (evita 502 do gateway Sanity com PNGs muito pesados da API). Depois define **texto alternativo** (`alt`) onde o schema o permitir; verifica **proporção** e revisão humana para marca e precisão.
