...
MCP local **`4unik-marketing`**: [`mcps/4unik-marketing/`](mcps/4unik-marketing/), ativar em [`.cursor/mcp.json`](.cursor/mcp.json). Tools de conhecimento: `get_notebooklm_briefing`, `search_product_knowledge`, `suggest_growth_opportunities`, `get_knowledge_freshness` (+ `get_ga4_metrics` via GA Data API quando credenciais locais, blog, registry). Snapshot tráfego: `npm run fetch:ga4-snapshot`. **GA4:** conta `66932658` (4unik), propriedade `327916606`, stream `G-SMJDYCENBC` — ver [`docs/knowledge-base/integrations.md`](docs/knowledge-base/integrations.md).
