...
- **Páginas de crescimento (pricing, segurança)**: rascunho com `robots: noindex` até aprovação em [`docs/content-approval-queue.md`](docs/content-approval-queue.md). Para indexar após sign-off: `NEXT_PUBLIC_INDEX_GROWTH_PAGES=true` no build + entradas no sitemap.
